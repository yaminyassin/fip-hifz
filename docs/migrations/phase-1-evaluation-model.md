...
13. **Participant import for the new event-scoped model:** `scripts/import_participants_from_csv.py` already writes to the event-scoped `events/{event_id}/participants` path (not a retired root collection), so its `insert-participants` script entry is kept, but the CSV/field shape it produces has not been reconciled against the V2 config-driven category and assignment model. Rewriting or validating it against V2 categories is future work.
