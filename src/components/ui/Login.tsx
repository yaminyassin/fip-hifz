...
export function Login() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            navigate({ to: `/?event=${eventId}` as any });
        } catch (err: any) {
            ...
            <Card className="w-full max-w-md p-6 space-y-6 bg-card/80 backdrop-blur-sm">
                <CardHeader className="p-0 mb-4 text-center">
                    <CardTitle className="text-2xl font-bold">
                        {eventId ? `Access ${eventId.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}` : t('login.title')}
                    </CardTitle>
                    <CardDescription>
                        {eventId ? `Enter the access code for ${eventId}` : t('login.subtitle')}
                    </CardDescription>
                </CardHeader>
