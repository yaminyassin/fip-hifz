...
export function TimePicker({ value = "", onChange }: TimePickerProps) {
  ...
  const [selectedHour, selectedMinute] = selectedTime.split(':').length === 2 
    ? selectedTime.split(':') 
    : ["00", "00"]
  ...
  React.useEffect(() => {
    if (hourRef.current) {
      ...
      if (hourElement) {
        hourRef.current.scrollTop = hourElement.getBoundingClientRect().top - 
    ...
    if (minuteRef.current) {
      ...
      if (minuteElement) {
        minuteRef.current.scrollTop = minuteElement.getBoundingClientRect().top - 
      ...
      <PopoverContent className="w-auto p-2" align="start">
        <div className="flex space-x-2">
          ...
          <div className="flex flex-col items-center">
            <div className="text-xs font-medium mb-1 text-muted-foreground">Hr</div>
            <div 
              ref={hourRef}
          ...
          {/* Minutes */}
          <div className="flex flex-col items-center">
            <div className="text-xs font-medium mb-1 text-muted-foreground">Min</div>
            <div 
              ref={minuteRef}
              className="w-14 h-32 overflow-y-auto scrollbar-thin relative"
              style={{
